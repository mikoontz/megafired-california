...
tuning_metrics_l <- lapply(biome_shortnames, FUN = function(biome_shortname) {
  tuning_metrics <- data.table::fread(input = paste0("data/out/rf/tuning/rf_ranger_spatial-cv-tuning-metrics_rtma_", biome_shortname, ".csv"))
  ...
  ggsave(filename = paste0("figs/informedness-vs-mcc_rtma_", biome_shortname, ".png"), plot = informedness_mcc_gg)
  ...
  ggsave(filename = paste0("figs/classification-threshold-effect_rtma_", biome_shortname, ".png"), plot = class_thresh_effect_gg)
