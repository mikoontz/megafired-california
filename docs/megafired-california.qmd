...
file.copy(
  from = "../tables/rf/early/2023-06-21/model-skill-results_early.csv", 
  to = "./tables/model-skill-results_early.csv"
)
file.copy(
  from = "../tables/rf/early/2023-06-21/model-skill-results_late.csv", 
  to = "./tables/model-skill-results_late.csv"
)

early_model_skill <- readr::read_csv("./tables/model-skill-results_early.csv")
late_model_skill <- readr::read_csv("./tables/model-skill-results_late.csv")
...
Fast fires are disproportionately impactful to ecosystems and society [@balch2024]. 
...
Wildfire has reached sizes and daily spread rates that are much higher than those seen throughout most of the 20th century, posing a growing risk to communities in the wildland urban interface (WUI). These fires are spreading despite record fire suppression efforts and costs (reaching \$4.4 billion in 2021 for Federal Suppression only [https://www.nifc.gov/fire-information/statistics/suppression-costs]).
Between 97 and 99% of fires are suppressed before reaching 120 hectares in size [@calkin2005], but the few that escape initial attack are simply spreading too fast and releasing too much energy to be fought effectively [@kreider2024].
Crucially, though large fires may burn for months, only a few days of extremely rapid spread generally account for most area burned [@coop2022; @balik2024], most severe ecological effects [@mcfarland2025], as well as the most detrimental social impacts [@balch2024].

For example, \>50% of the 4 million hectares burned in the United States in 2020 burned in just the largest 5% of fire-days [@coop2022], and the top 3% of fastest daily spread events accounted for 15-55% of total area burned across ecoregions in North America [@balik2024].
Extreme spread events also result in greater fire severity, and more homogeneous arrangements of high-severity fire with dire consequences for forest regeneration in the southwestern United States [@mcfarland2025].
Finally, fire events that include one of the fastest 2.7% of daily growth account for 78% of structure loss, 82% of structure damage, 43% of injuries, and 66% of fatalities [@balch2024].
To better anticipate, mitigate, and prepare for these fast fires, it is critical to determine what conditions amplify normal fire spread into extreme fire spread.
...
Because fast fire dynamics can differ by different vegetation types [@balik2024], it is important to test whether any of the drivers of rapid spread also differ.
Extreme spread events can be especially dangerous if they occur within the first several days of ignition of a fire, because resources and communications links are often not as robust as later on in the lifespan of a major wildfire; so it would be valuable to know if there are any drivers distinctly associated with early rapid spread. 
...
Previous research has found strong associations between the rate of fire spread, fire intensity, and fire severity [@birch2014; @birch2015; @hart2020; @mcfarland2025].
...
To identify factors that distinguish "normal active spread" from "extreme spread", we directly contrast places and times of extreme fire spread to those where fire is already burning actively, but not spreading at an extremely fast rate.
...
In this study, we use daily time-step fire data generated using the FIRED algorithm (@balch2020) to investigate drivers of extremely rapid spread in California from 2003-2020.
...
We build on previous research that suggests rapid spread dynamics may be different in forests vs other Mediterranean-climate vegetation (woodland, scrubland, grassland) to investigate whether the predictors of rapid spread differ in important ways between these two vegetation classes.
...
1. What are the conditions of active fire spread?
1. What are the conditions that distinguish extreme fire spread from active fire spread, and what are the relative importance of different classes of drivers including weather, fuels, topography, and human factors?
1. Are extreme spread events driven by different factors in different ecosystems with distinct fire regimes (in particular, temperate conifer forests vs Mediterranean shrublands)
1. Is there a distinct set of factors associated with escape from containment and extremely rapid spread early on in the life of a wildfire (i.e., within the first week) compared to later on?
1. Finally, what is the relative hazard of extreme fire spread across potential operational delineations (i.e., PODs [@thompson2022]) in California’s Sierra Nevada forests, and how does this compare to previous work @peeler2023 mapping extreme fire hazard?

[Points from before to perhaps include]
[Could use a justification for why finer scale is important]
...
[Lit review 1: Describe and distinguish the scope of this paper] 
In recent years, researchers have taken advantage of the ready availability of increasingly long records of fire extent coupled with more comprehensive datasets on potential drivers to identify factors most strongly associated with wildfire area burned, mostly at regional scales.
...
For each spread event, we used the Resolve biome map to assign a biome based on greatest fractional coverage: 1) temperate conifer forests, 2) Mediterranean forests, woodlands, and scrub 3) temperate grasslands, savannas, and shrublands, and 4) deserts and xeric shrublands (Resolve 2017).
To focus specifically on early or "surprise" spread events, we divided events into two temporal contexts: “early” spread events occurred within the first seven days of the fire event, and “late” spread events occurred after the first seven days of the fire event.
...
### Obtaining percentile values for weather variables

For each weather variable that was normalized, we generated a 401-layer raster at the data product’s original resolution where each layer represented the value of that weather variable that corresponded to a particular percentile (0th through 100th percentiles in increments of 0.25) of all values of the variable aggregated over the 40-year period between January 1, 1981 and December 31, 2020.
Then, for each daily fire polygon location, we assigned a percentile by matching the extracted weather variable value on the day of the spread event to the value closest to it in the 401-layer raster.

### Correcting scale-dependence in fuel and topography variables

Scale-dependence is a potential challenge for using mapped variables to distinguish small from large events, since the values of these variables may depend on the area sampled (e.g., extreme values such as 0% or 100% of a cover type or topographic feature are more likely to occur in smaller samples). 
We developed a randomization procedure to deal with this kind of scale-dependence.
...
Further, we calculated the total cover land area within each fire independent polygon (i.e., total area minus the cover of water bodies), and used it as the denominator for calculating fractional cover of fire independent fuel and topography variables.
...
[placeholder for using the "random" approach for tie breaking]
...
Some spread events are single polygons, while others are multipolygons that could be widely dispersed (for instance, multiple leading edges of a very large spreading fire that burned on the same day would be represented by multiple polygons for that daily spread event and could be many kilometers apart).
For this reason, when extracting weather variables at a single point, we used the spatial centroid of the daily polygon in the case of single-polygon geometries and used the spatial centroid of the largest polygon for multipolygon geometries.
We extracted weather data derived from the ERA5-Land reanalysis for each of the 24 observations within the day that the spread event represented [@hersbach2020] and then summarized those values as described below to produce a single daily value.
...
We visualized the distribution and calculated the mean value of all predictor variables across the two remaining biomes (temperate conifer forests and Mediterranean forest, woodland, and scrub) and the two temporal contexts (early, later).
...
Extreme events are rare by definition: the "extreme spread event" category contains 5% of data while the "normal spread event" category contains 95% of data.
To improve how the models hanlde unbalanced data, we upsampled the extreme class of events by using class weights for the extreme spread events class as the reciprocal of the number of non-extreme spread events in the training data, and vice versa.
...
MCC is particularly well-suited to binary classification problems with imbalanced class representation as is the case here [@chicco2021; @poisot2022].
...
[placeholder for explanation of why some spatial folds have no MCC value; need to double check why. I think because some didn't have all components? true positive, true negative, false positive, false negative?]
...
Relative hazard was then determined by fitting the most skillful model to each set of observed feature values for each POD (i.e., the percentile ranked actual measures of the fuel and topography features within each POD and the fixed values of the human, weather, and fire features).
...
We found that relatively extreme weather conditions are typical for any kind of fire spread (i.e., both extreme and non-extreme)

![Maximum temperature is extremely high.](figs/covariate-histograms/max_temp_era5_pct.png){#fig-max-temp}

![Minimum relative humidity is extremely low.](figs/covariate-histograms/min_rh_era5_pct.png){#fig-min-rh}

![Maximum vapor pressure deficit is extremely high.](figs/covariate-histograms/max_vpd_era5_pct.png){#fig-max-vpd}

![100-hour fuel moisture is extremely low.](figs/covariate-histograms/fm100_pct.png){#fig-fm100}

![1000-hour fuel moisture is extremely low.](figs/covariate-histograms/fm1000_pct.png){#fig-fm1000}

![Maximum wind speed is extremely high.](figs/covariate-histograms/max_wind_speed_era5_pct.png){#fig-max-wind-speed}

![Burning index is high during all active fire spread events](figs/covariate-histograms/bi_pct.png){#fig-bi}

![Energy Release Component is extremely high.](figs/covariate-histograms/erc_pct.png){#fig-erc}

Maximum temperature and maximum wind speed are extremely high during active fire spread (@fig-max-temp; @fig-max-wind-speed).
Fuel moisture conditions are extremely low during active fire spread (@fig-fm100; @fig-fm1000).
Minimum relative humidity is extremely low and vapor pressure deficit is extremely high during active fire spread (@fig-min-rh; @fig-max-vpd).
Integrated measures of fire potential, such as the energy release component and burning index are both extremely high during active fire spread (@fig-bi; @fig-erc)
...
model_skill_summary_table <- rbind(
  dplyr::mutate(early_model_skill, `Temporal context` = "early"),
  dplyr::mutate(late_model_skill, `Temporal context` = "late")
) |> 
  dplyr::mutate(
    `Resolve biome name` = dplyr::case_when(
      biome == "dxs" ~ "Deserts and xeric shrublands",
      biome == "tcf" ~ "Temperate conifer forests",
      biome == "mfws" ~ "Mediterranean forests, woodland, and scrub"
    )
  ) |>
  dplyr::filter(biome != "dxs") |> 
  dplyr::mutate(
    `Matthew's Correlation Coefficient (MCC)` = stringr::str_pad(
      string = round(
        mean_mcc, 
        digits = 3
      ), 
    ...
    `Classification threshold` = stringr::str_pad(
      string = classification_thresh,
      side = "right",
      pad = "0",
      width = 4
    )
...
Our spatially cross-validated random forest model achieved a model skill ranging from `r min(unlist(model_skill_summary_table[, "Matthew's Correlation Coefficient (MCC)"]))` to `r max(unlist(model_skill_summary_table[, "Matthew's Correlation Coefficient (MCC)"]))` as measured by Matthew's Correlation Coefficient, a conservative measure that accounts for both correct and missed classification of both the potential binary outcomes (Table 2).

Table 2. Tuned random forest hyperparameters and model skill based on Matthew's Correlation Coefficient for the four unique combinations of biome/temporal context.
...
> We can't put out all the fires that we should, and we shouldn't put out all the fires that we can.
