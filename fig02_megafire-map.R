...
resolve <-
  ...
  dplyr::mutate(biome_name = as.factor(BIOME_NAME))

events <-
  sf::st_read("incubator/fired_events_ca_ewe_rank_v5.gpkg") %>%
  sf::st_transform(3310) %>%
  sf::st_centroid() %>%
...
plot(st_geometry(ca))
plot(events[, "megafire"], add = TRUE, pal = c("red", "black"), pch = 19, cex = 0.5)
...
test <-
  resolve %>% sf::st_collection_extract(type = "LINESTRING")

test2 <- rbind(resolve_ca, test)

megafire_resolve_map <-
  tm_shape(ca) +
  tm_borders() +
  tm_shape(resolve_ca) +
  tm_polygons(col = "BIOME_NAME", title = "Resolve biome name") +
  tm_shape(events) +
  tm_dots(col = "megafire", palette = c("red", "black"), title = "Fire type", legend.is.portrait = TRUE, size = 0.05) +
  tm_graticules(alpha = 0.5, lines = FALSE)

# +
#   tm_layout(legend.outside.position = "bottom", legend.outside = TRUE, legend.text.size = 1, legend.outside.size = 1)

megafire_resolve_map

tmap::tmap_save(tm = megafire_resolve_map, filename = "figs/megafire-resolve-map.jpg", width = 180, height = 220, units = "mm", dpi = 300)
...
megafire_map <-
  ...
  tm_layout(legend.text.size = 1, legend.bg.color = "white", legend.frame = TRUE, legend.title.size = 1.25)
...
tmap::tmap_save(tm = megafire_map, filename = "figs/megafire-map.jpg", width = 180, height = 220, units = "mm", dpi = 300)
...
# 
# peak/ridge = c(11, 12, 13, 14, 15)
# upper slope = c(21, 22, 23, 24)
# lower slope = c(31, 32, 33, 34)
# valley = c(41, 42)
...
fires_working <-
  ...
  sf::st_drop_geometry() %>% 
  dplyr::select(-code, -
                  -lcms_landuse_01, -lcms_landuse_02, -lcms_landuse_04, -lcms_landuse_07,
...
(peak_ridge_cols <- paste0("csp_ergo_landforms_", 11:15, "_prop") %>% paste(collapse = " + "))
(upper_slope_cols <- paste0("csp_ergo_landforms_", 21:24, "_prop") %>% paste(collapse = " + "))
(lower_slope_cols <- paste0("csp_ergo_landforms_", 31:34, "_prop") %>% paste(collapse = " + "))
(valley_cols <- paste0("csp_ergo_landforms_", 41:42, "_prop") %>% paste(collapse = " + "))

fires_working %>% 
  dplyr::mutate(peak_ridge_prop = (csp_ergo_landforms_11_prop + csp_ergo_landforms_12_prop + csp_ergo_landforms_13_prop + csp_ergo_landforms_14_prop + csp_ergo_landforms_15_prop) / 0.1156,
                upper_slope_prop = (csp_ergo_landforms_21_prop + csp_ergo_landforms_22_prop + csp_ergo_landforms_23_prop + csp_ergo_landforms_24_prop) / 0.1156,
                lower_slope_prop = (csp_ergo_landforms_31_prop + csp_ergo_landforms_32_prop + csp_ergo_landforms_33_prop + csp_ergo_landforms_34_prop) / 0.1156,
                valley_prop = (csp_ergo_landforms_41_prop + csp_ergo_landforms_42_prop) / 0.1156) %>% 
  dplyr::select(-starts_with("csp_ergo_landforms"))

test <- fires_working %>% 
  dplyr::select(starts_with("csp_ergo_landforms")) %>% 
  dplyr::select(ends_with("prop"))
