...
latest_rf_tuning_dir <- here::here("data", "out", "rf", "tuning", "early", latest_ard_date)
latest_rf_figs_dir <- here::here("figs", "rf", "tuning", "early", latest_ard_date)
...
pbapply::pblapply(X = biome_shortnames, FUN = function(biome_shortname) {
  biome_tune <- data.table::fread(here::here(latest_rf_tuning_dir, paste0("rf_ranger_spatial-cv-tuning_rtma_", biome_shortname, "_early.csv")))
...
tuning_metrics_l <- lapply(biome_shortnames, FUN = function(biome_shortname) {
  ...
  ggsave(filename = here::here(latest_rf_figs_dir, paste0("informedness-vs-mcc_rtma_", biome_shortname, "_early.png")), plot = informedness_mcc_gg)
  ...
  ggsave(filename = here::here(latest_rf_figs_dir, paste0("classification-threshold-effect_rtma_", biome_shortname, "_early.png")), plot = class_thresh_effect_gg)
  
})
(end_time <- Sys.time())
(difftime(end_time, start_time, units = "hours"))

# Time difference of 0.9394318 hour on a 12-core machine with 64GB of RAM
