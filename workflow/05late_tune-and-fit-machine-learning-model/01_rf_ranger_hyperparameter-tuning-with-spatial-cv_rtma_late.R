...
local_out_dir <- here::here("data", "out", "rf", "tuning", "late", latest_ard_date)
...
for(counter in seq_along(biome_shortnames)) {
  ...
  data <- read.csv(paste0(latest_ard_dir, "/daily-drivers-of-california-megafires_", biome_shortname,"_late.csv"))
  ...
  data.table::fwrite(x = out_all, file = paste0(local_out_dir, "/rf_ranger_spatial-cv-tuning_rtma_", biome_shortname, "_late.csv"))
}
(end_time <- Sys.time())
(difftime(end_time, start_time, units = "hours"))

# 12-core machine with 64GB of RAM for 2023-06-21 late fire version
# [1] "Starting the tcf biome at 2023-06-21 21:42:25"
# |++++++++++++++++++++++++++++++++++++++++++++++++++| 100% elapsed=04h 34m 19s
# [1] "Starting the mfws biome at 2023-06-22 02:16:45"
# |++++++++++++++++++++++++++++++++++++++++++++++++++| 100% elapsed=01h 36m 30s
# [1] "Starting the dxs biome at 2023-06-22 03:53:15"
# Error in `rsample::make_splits()`:
#   ! The analysis set must contain at least one row.
# Run `rlang::last_trace()` to see where the error occurred.
# The DXS biome only has one spatial fold that met the minimum data requirements, so it isn't
# possible to do the spatial cross validation here