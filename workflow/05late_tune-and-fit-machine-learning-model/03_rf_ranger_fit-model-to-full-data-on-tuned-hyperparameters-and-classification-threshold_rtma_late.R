...
rf_tuning_dir <- here::here("data", "out", "rf", "tuning", "late", latest_ard_date)
...
rf_predict_dir <- here::here("data", "out", "rf", "predict", "late", latest_ard_date)
...
tuning_metrics_l <- lapply(biome_shortnames, FUN = function(biome_shortname) {
  data.table::fread(input = here::here(rf_tuning_dir, paste0("rf_ranger_spatial-cv-tuning-metrics_rtma_", biome_shortname, "_late.csv")))
})
...
write.csv(x = tuned_hyperparameters, file = here::here(rf_tables_dir, "model-skill-results_late.csv"))
...
for(counter in seq_along(biome_shortnames)) {
  ...
  readr::write_rds(x = fitted_model, file = here::here(rf_fitted_dir, paste0("rf_ranger_fitted-model_rtma_", biome_shortname, "_late.rds")))
}
...
fitted_models_l <- lapply(X = biome_shortnames, FUN = function(biome_shortname) {
  return(readr::read_rds(file = here::here(rf_fitted_dir, paste0("rf_ranger_fitted-model_rtma_", biome_shortname, "_late.rds"))))
})
...
for(counter in seq_along(biome_shortnames)) {
  ...
  if(!file.exists(here::here(rf_predict_dir, paste0("rf_ranger_predictions_rtma", biome_shortname, "_late.csv")))) {
    set.seed(1510)
    ...
    data.table::fwrite(x = newdata, file = here::here(rf_predict_dir, paste0("rf_ranger_predictions_rtma_", biome_shortname, "_late.csv")))
  }
}
