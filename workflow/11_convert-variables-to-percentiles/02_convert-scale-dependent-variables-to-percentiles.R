...
fired_daily_resolve <- 
  ...
  dplyr::select(did, id, date, biome_shortname)

landfire_disturbance_version <- "v1"

fired_drivers_fname <- "data/out/drivers/fired-fluc-static-driver-proportions.csv"
fi_drivers_fname <- "data/out/drivers/fi-fluc-static-driver-proportions.csv"

out_fname <- "data/out/drivers/fluc-static-driver-proportion-percentiles.csv"

# Get drivers as measured in fire-independent fire footprints
fi_drivers_all <- data.table::fread("data/out/drivers/fi-fluc-static-driver-proportions.csv")

# Get drivers as measured from within fire footprints
fired_drivers_all <- data.table::fread("data/out/drivers/fired-fluc-static-driver-proportions.csv")

for(i in seq_along(biome_name_lookup$biome_shortname)) {
  
  biome_shortname <- biome_name_lookup$biome_shortname[i]
  biome_idx <- fired_daily_resolve[fired_daily_resolve$biome_shortname == biome_shortname, "did"]
  fi_drivers <- fi_drivers_all[did %in% biome_idx, ]  
  fi_drivers[, `:=`(date = as.Date(date),
                    samp_id = NULL)]
  
  # data.table wide to long format reshaping (tidyr::tidy_longer)
  # https://cran.r-project.org/web/packages/data.table/vignettes/datatable-reshape.html
  fi_drivers_long <- data.table::melt(data = fi_drivers, id.vars = c("did", "date", "id"))

  # data.table nesting (tidyr::nest)
